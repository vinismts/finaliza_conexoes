...
if not processos_java:
    logging.debug('Não foram localizados PIDs para serem encerrados.')
    sys.exit()
else:
...
#Obtem os dados do processo java.exe, tenta comunicar-se com o ip\porta informado nos dados do 
                        ...
                        Telnet(ip,porta)
                        ip_porta[p[comando1]]=p[comando2]
...
#Checa se houve algum PID finalizado, caso não, grava no log informando
    if qtd_exceptions == 0 and ip_porta:
        logging.debug('Não foram localizados PIDs para serem encerrados.')
        sys.exit()
